<template>
  <div class="user">
    <el-card class="box-car">
      ...
      <!--表格-->
      <el-table class="needLoading" v-auto-height="{ bottom: 80 }" :data="tableData" style="width: 100%"  border>
...
const getTableDataByPage = async () => {
  // 开启loading
  global.$myLoading.showFullScreenLoading('.needLoading', '正在载入数据')
  await getAllUserInfoByPageApi(pageNo.value, pageSize.value).then(res => {
    if (res && res.data) {
    ...
    // 关闭loading
    global.$myLoading.hideFullScreenLoading()
  })
}
// 组件挂载
onMounted(async () => {
...
//   tableHeight.value = screenHeight.value - 280
// }
//
// // 在组件销毁前移除事件监听器
// onBeforeUnmount(() => {
...
// })
//
// // 监听screenHeight的变化，自动更新tableHeight
// watch(screenHeight, () => {
//   tableHeight.value = screenHeight.value - 280
// })
</script>
